...
func json_handle(ws *websocket.Conn, j map[string]interface{}) {
	if j["func"] == "wb_register" {
		...
		if status == "on" {
			wsc.TURN_ON(hw_list, target)
			//go linebot("[APW]遠端關閉電源【" + target + "】")
		}
		if status == "off" {
			wsc.TURN_OFF(hw_list, target)
			//go linebot("[APW]遠端開啟電源【" + target + "】")
		}
		log.Printf("send switch uuid = %s, switch = %s", target, status)
	} else if j["func"] == "wb_toggle" {
		target, _ := j["uuid"].(string)
		wsc.TURN_TOGGLE(hw_list, target)
		log.Printf("send switch uuid = %s, toggle", target)
	} else if j["func"] == "wb_status" {
		target, _ := j["uuid"].(string)
		wsc.TURN_STATUS(hw_list, target)
		log.Printf("send switch uuid = %s, status", target)
	}
}
